...
export default {
  ...
  mounted() {
    ...
    this.$http
      .get("/homework1_4_war/rest/demo/query_user_by_id?state=4")
      .then(res => {
        const data = res.data;
        this.driverSchool = data.value.filter(item => item.business == "驾校");
        this.drugstore = data.value.filter(item => item.business == "药店");
        this.hotel = data.value.filter(item => item.business == "酒店");
        this.init();
      });
    // }
    // catch (e) {
  ...
  methods: {
    ...
    searchSubmit() {
      ...
      this.$http
        .get(
          "/homework1_4_war/rest/demo/query_user_by_id?user_id=" +
            this.searchIdTelName +
            "&state=" +
            this.searchState
        )
        .then(res => {
